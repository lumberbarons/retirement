...
- The household's pre-retirement working years are in scope, not just decumulation: each spouse's retirement age (default 65) and earned income to retirement are config inputs; earned income covers baseline spending before retirement, drives RRSP room accrual (T024) and, while a spouse is an active DB member, the pension adjustment (T020); each spouse's own-income RRSP/TFSA contributions follow simple config-set annual amounts, with any surplus net earned income flowing to non-registered savings. What stays out of scope is specifically the *splitting* vehicles built on top of that — spousal RRSP elections, prescribed-rate loans, TFSA gifting, expense shifting — not the underlying earned-income and contribution mechanics
- In the gross-vs-net bisection solve, discretionary withdrawals across spouses within each asset tier (non-registered, RRSP/RRIF, TFSA) are drawn proportionally to each spouse's balance in that tier (pro-rata across spouses, with single-owner accounts drawn solely by the owner), ensuring deterministic convergence
- Non-refundable tax credits between spouses model both the federal and Ontario spousal amounts (BPA equivalent reduced by spouse's net income) and the transfer of unused age and pension income amounts (Schedule 2) to the higher-income spouse
...
The config covers spouses (birth year, retirement age, death age, earned income
to retirement, CPP entitlement at 65 and start age, OAS start age, DB pension
terms), accounts by type and owner
(TFSA, RRSP/RRIF with spousal tags and contribution years, non-registered
with ACB and asset-yield parameters), and the spending target with its mode.
Every dollar constant the engine uses lives in dated tables keyed by year with
their index basis — not scattered literals — and a bad config fails with an
error naming the first offending field, not a stack trace.
...
age) and a survivor spending factor of ~70% on first death. The solve bisects
gross withdrawals within the baseline priority order (non-registered →
RRSP/RRIF → TFSA) drawn proportionally to each spouse's balance within each
tier to a $1 tolerance — monotonic, so it converges even in the OAS clawback
zone where the effective marginal rate exceeds 60%.
...
credits (the ordering moves a retiree between surtax bands), the pension
income amount carries per-person, per-income-type eligibility flags (DB
qualifies at any age, RRIF at 65+, RRSP withdrawals and CPP/OAS never), and
unused age and pension credits transfer to the higher-income spouse alongside
the spousal amount. Partial non-registered withdrawals realize capital gains
proportionally against ACB (50% inclusion) and decrement remaining ACB. Every
...
household — they can reappear for a low-income survivor in late life.
...
little or no top-up), CPP pays the $2,500 lump-sum death benefit, brackets and
credits go single, splitting stops, and spending drops to the survivor factor.
At second death the entire remaining RRIF is included in the death-year return
(often ~53.53%), non-registered assets are deemed disposed at FMV, the terminal
$2,500 CPP death benefit applies, and the summary surfaces terminal tax and
...
- [ ] T002 Define the household config schema (spouses incl. retirement age and earned income, accounts with ACB and yields, benefits, pensions, spending) — `internal/config/schema.go`
...
- [ ] T010 Implement the gross-vs-net bisection over withdrawals in the baseline priority order (non-registered → RRSP/RRIF → TFSA), allocating pro-rata across spouses — `internal/projection/solve.go`

### US4

- [ ] T011 Implement federal and Ontario bracket engines indexed from the 2026 base — `internal/tax/brackets.go`
- [ ] T012 Implement the Ontario surtax on basic tax after credits, the health premium, and non-refundable credits (spousal amount, pension-amount eligibility, and unused credit transfers) — `internal/tax/ontario.go`
- [ ] T013 Implement eligible/non-eligible dividend gross-up and DTCs, annual investment yields, and proportional capital-gains realization against ACB (50% inclusion) on non-registered withdrawals — `internal/tax/income.go`
...
- [ ] T025 Implement first-death events: TFSA successor-holder merge, RRIF rollover, one OAS lost, CPP survivor under the cap, $2,500 CPP death benefit, single brackets/credits, splitting stopped, survivor spending factor — `internal/projection/death.go`
- [ ] T026 Implement the second-death terminal year: full RRIF inclusion, deemed disposition at FMV, terminal $2,500 CPP death benefit, terminal tax and estate value in the summary — `internal/projection/terminal.go`
...
- [ ] A run with unequal spouse incomes demonstrates transfer of unused non-refundable tax credits (age or pension amount) to the higher-income spouse, shown by a unit test
- [ ] A non-registered withdrawal triggers capital gains proportional to `(1 - ACB/balance)` at 50% inclusion and reduces ACB proportionally, verified by a unit test
- [ ] First death processes the $2,500 lump-sum CPP death benefit as taxable income of the terminal year
