...
- Accumulation-phase splitting vehicles — new spousal RRSP contributions, prescribed-rate loans, spousal TFSA gifting, expense shifting — a later epic; withdrawal-time attribution of accounts already declared spousal in config IS in scope
- LIRA/LIF mechanics — no commuted value is planned; revisit if that changes
- Stochastic modelling: Monte Carlo, historical bootstrap, sequence-of-returns reporting, probabilistic (CPM2014) mortality, the long-term-care shock, and the annuity/ALDA purchase option
...
The config covers spouses (birth year, death age, CPP entitlement at 65 and
start age, OAS start age, DB pension terms), accounts by type and owner
...
roof, vehicle replacements) and a survivor spending factor of ~70% on first
death. The solve bisects gross withdrawals within the baseline priority order
...
65–74, $160,647 at 75+). GIS is evaluated every year even though it is
usually zero for this household — it can reappear for a low-income survivor
in late life.
...
contributions (a 2026 withdrawal restores room on Jan 1, 2027), and the
accumulation default fills both TFSAs to the annual limit regardless of whose
income funds them. RRSP converts to RRIF by Dec 31 of the year the holder
...
- [ ] T002 Define the household config schema (spouses, accounts, benefits, pensions, spending) — `internal/config/schema.go`
...
- [ ] T019 Implement the DB pension stream (formula, bridge-to-65, indexation, J&S reduction, survivor percentage) and the PA that reduces RRSP room — `internal/pension/db.go`

### US7

- [ ] T020 Implement the TFSA room engine with Jan-1 prior-year-withdrawal recontribution and default-fill contributions — `internal/accounts/tfsa.go`
- [ ] T021 Implement RRSP→RRIF conversion at 71, Jan-1 minimum factors, younger-spouse election, and withholding as prepayment — `internal/accounts/rrif.go`
- [ ] T022 Implement spousal-RRSP attribution with the 3-year window and RRIF-minimum carve-out — `internal/accounts/attribution.go`
- [ ] T023 Implement RRSP room accrual (18% of earned income, dollar limit, carry-forward) and accumulation-phase contributions — `internal/accounts/rrsp.go`

### US8

- [ ] T024 Implement first-death events: TFSA successor-holder merge, RRIF rollover, one OAS lost, CPP survivor under the cap, single brackets/credits, splitting stopped, survivor spending factor — `internal/projection/death.go`
- [ ] T025 Implement the second-death terminal year: full RRIF inclusion, deemed disposition at FMV, terminal tax and estate value in the summary — `internal/projection/terminal.go`
